...
object KeyLabLogger {
    ...
    fun log(type: String, value: String? = null, layout: String = "unknown", source: String = "typed") {
        try {
            val ts = System.currentTimeMillis()
            val safeValue = value?.replace("\"", "\\\"")
            val line = if (safeValue != null) {
                """{"ts":$ts,"type":"$type","val":"$safeValue","layout":"$layout","source":"$source"}"""
            } else {
                """{"ts":$ts,"type":"$type","layout":"$layout","source":"$source"}"""
            }
